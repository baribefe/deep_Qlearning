...
class neural_network(object):
    ...
    def __init__(self, hidden1=500, hidden2=500, hidden3=500, n_classes=4, batch_size=100, nvariables=8):
        ...
        self.out={'weights':tf.Variable(tf.random_normal([self.hidden3,self.n_classes])),'biases': tf.Variable(tf.random_normal([self.n_classes]))}
    ...
    def train_neural_network(self,sess,dataX,dataY,alpha):
        #sess.run(self.optimizer, feed_dict = {self.X: dataX, self.Y: dataY.eval()})
        self.optimizer = tf.train.GradientDescentOptimizer(learning_rate=alpha).minimize(cost)
	...
	to_copy = neural_net.variables
        self.ly1['weights'].assign(to_copy[0])
        self.ly1['biases'].assign(to_copy[1])
        self.ly2['weights'].assign(to_copy[2])
        self.ly2['biases'].assign(to_copy[3])
        self.ly3['weights'].assign(to_copy[4])
        self.ly3['biases'].assign(to_copy[5])
        self.out['weights'].assign(to_copy[6])
        self.out['biases'].assign(to_copy[7])
